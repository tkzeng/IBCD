...
def matrix_model(obs_data, pi_0, pi_k, sigma_grid, U_lower, V_lower, D, sigma0=0.001, epsilon=1e-5):
    """
    Define a NumPyro probabilistic model with a spike-and-slab prior for G
    and matrix-normal likelihood for the observed data.

    Parameters:
        obs_data: Observed data matrix
        pi_0: Optimized weight for spike component
        pi_k: Optimized weights for slab components
        sigma_grid: Slab variances
        sigma0: Variance for spike component
        U_lower: Lower triangular Cholesky factor of row covariance
        V_lower: Lower triangular Cholesky factor of column covariance
        D: Dimensionality of the matrix
        epsilon: Regularization term to ensure stability
    """

    pi_0 = jnp.array([pi_0])
    pi_k = jnp.array(pi_k)
    
    # Define the mixing distribution (spike + slab)
    mixing_dist = dist.Categorical(probs=jnp.concatenate([pi_0, pi_k]))
    
    # Define the spike and slab components
    component_dists = [
        dist.Normal(loc=0.0, scale=sigma0),  # Spike component
        *[
            dist.Normal(loc=0.0, scale=sigma_k)  # Slab components
            for sigma_k in sigma_grid
        ]
    ]

    # Combine the components into a mixture
    G_prior = dist.MixtureGeneral(mixing_dist, component_dists).expand((D, D))

    # Sample G from the spike-and-slab prior
    G = numpyro.sample('G', G_prior)

    # Regularized inverse calculation for (I - G)
    I = jnp.eye(D)
    I_minus_G = I - G + epsilon * jnp.eye(D)
    #R_mean = jnp.linalg.inv(I_minus_G)
    R_mean = jnp.linalg.solve(I_minus_G, I)
    #R_mean = inverse_approximation(I_minus_G, method="cg")
...
def matrix_model_edge_specific2(
    obs_data,        # Observed data matrix
...
def matrix_model_edge_specific(
    ...
    pi0_ij,              # (D,D) array of edge-specific spike weights
    pi_k_ij,             # (D,D) array of edge-specific total slab portion
    pi_k,                # (K,) global slab mixture weights (these sum to 1 - pi0)
    sigma_grid,          # (K,) list of slab std devs
    U_lower, V_lower,    # row/col cholesky factors for MatrixNormal
    D,
    sigma0=0.001,
    epsilon=1e-5
):
    """
    If pi0_ij and pi_k_ij are constant across (i,j), this should match the global model.
    """

    # 1) Mixture weights: shape (D,D,K+1)
    # spike_probs = pi0_ij[..., None]                      # (D,D,1)
    # slab_probs_k = pi_k_ij[..., None] * pi_k[None, None, :]  # (D,D,K)
    # mixing_probs = jnp.concatenate([spike_probs, slab_probs_k], axis=-1)  # (D,D,K+1)

    # # 2) Categorical with batch shape (D,D)
    # mixing_dist = dist.Categorical(probs=mixing_probs)

    # # 3) Use shared (not expanded) component distributions
    # component_dists = [dist.Normal(0.0, sigma0)]
    # for s_k in sigma_grid:
    #     component_dists.append(dist.Normal(0.0, s_k))

    # # 4) MixtureGeneral with shared components, then expand to (D,D)
    # G_prior = dist.MixtureGeneral(mixing_dist, component_dists).expand((D, D))

    spike_probs = pi0_ij[..., None]  # shape (D,D,1)

    pi_k_norm = pi_k / jnp.sum(pi_k)  # shape (K,)
    slab_probs = (1.0 - pi0_ij)[..., None] * pi_k_norm[None, None, :]  # shape (D, D, K)

    # slab_probs = pi_k_ij[..., None] * pi_k[None,None,:]  # shape (D,D,K)
    mixing_probs = jnp.concatenate([spike_probs, slab_probs], axis=-1)  # shape (D,D,K+1)

    # 2) A categorical distribution with batch_shape=(D,D).
    mixing_dist = dist.Categorical(probs=mixing_probs)

    # 3) Each component distribution must also have batch_shape=(D,D).
    #    We'll build them expanded:
    comp_dists = []
    comp_dists.append(dist.Normal(0.0, sigma0).expand((D,D)))  # spike
    for s_k in sigma_grid:
        comp_dists.append(dist.Normal(0.0, s_k).expand((D,D))) # each slab

    # 4) Combine them in a MixtureGeneral.  Because both mixing_dist and
    #    comp_dists already have batch_shape=(D,D), we do *not* call expand((D,D)) again.
    G_prior = dist.MixtureGeneral(mixing_dist, comp_dists)
    
    # 5) Sample G from this edge-specific prior
    G = numpyro.sample("G", G_prior)

    # 6) Compute regularized inverse of (I - G + eps)
    I = jnp.eye(D)
    I_minus_G = I - G + epsilon * jnp.eye(D)
    R_mean = jnp.linalg.solve(I_minus_G, I)

    # 7) MatrixNormal likelihood
    numpyro.sample(
        "R_hat_obs",
        dist.MatrixNormal(
            loc=R_mean,
...
def load_R_and_SE_hat(r_hat_file, se_hat_file):
    """
    Load R_hat and SE_hat matrices, extract upper triangle off-diagonal entries.
    
    Parameters:
        r_hat_file (str): CSV file for R_hat.
        se_hat_file (str): CSV file for SE_hat.

    Returns:
        w (jnp.ndarray): Off-diagonal R_hat entries.
        se (jnp.ndarray): Off-diagonal SE_hat entries (corresponding to w).
    """
    ...
    #w = R_hat[jnp.triu_indices(N, k=1)]
    #se = SE_hat[jnp.triu_indices(N, k=1)]
    mask = ~np.eye(N, dtype=bool)
    w = R_hat[mask]
    se = SE_hat[mask]
    
    print(f"Shape of w: {w.shape}, Shape of se: {se.shape}")
    return w, se

def empirical_bayes_em(w, se_hat, K=50, sigma_min=0.01, sigma_max=1.0, alpha=2, max_iter=40000, tol=1e-6):
    """
...
if __name__ == "__main__":
 # path = "/home/seong/lr/dontmessup/50d_test/parallel/edge/" 
  path = "/your/path/"
  print("Default device:", jax.devices()[0])

  #w = load_R_hat("/Users/seongwoohan/Desktop/bcbg/data/random1_500/R_hat_files500_random1/R_hat1_1.csv")  
  w, se_hat = load_R_and_SE_hat("R_true.csv", "SE_hat.csv")

  print(w.shape)
  #pi0, pi_slabs, slab_scales = empirical_bayes_em(w, alpha=2)
  pi0, pi_slabs, slab_scales = empirical_bayes_em(w, se_hat, alpha=2)
  ...
  # Rhat_df = pd.read_csv("R_mean_random1_500_old_mle.csv", header=None).values
  #U_df = pd.read_csv("U_row_random1_500_old_mle.csv", header=None).values
  #V_df = pd.read_csv("V_column_random1_500_old_mle.csv", header=None).values
  ...
  #Rhat_mat = pd.read_csv("R_hat_represent.csv", header=None).to_numpy(dtype=float)
  Rhat_df = pd.read_csv("R_true.csv")
  # (2) Read directly to numpy and skip .values entirely:
  U_mat = pd.read_csv("U_true.csv")
  V_mat = pd.read_csv("V_true.csv")
  U_lower = jnp.linalg.cholesky(jnp.array(U_mat.values, dtype=jnp.float64))
  V_lower = jnp.linalg.cholesky(jnp.array(V_mat.values, dtype=jnp.float64))
  #U_lower = jnp.linalg.cholesky(jnp.array(U_mat))
  #V_lower = jnp.linalg.cholesky(jnp.array(V_mat))

  # MCMC setup
  kernel = NUTS(matrix_model_edge_specific, target_accept_prob=0.7, max_tree_depth=10, init_strategy=infer.init_to_median(num_samples=3))
...
#   )
  ...
  mcmc_sampler.run(
      jax.random.PRNGKey(42),
      obs_data=Rhat_df.values, #Rhat_mat, # Rhat_df.values,
      pi0_ij= pi0_ij,
      pi_k_ij=pi_k_ij,
  ...
  np.save(os.path.join(path, "flat_samples.npy"), flat_samples)
